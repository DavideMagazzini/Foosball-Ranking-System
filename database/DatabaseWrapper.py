...
class DatabaseWrapper():
    ...
    def getPlayerAchievementData(self, playerId: str | ObjectId, achievementId: str | ObjectId) -> dict | None:
        """
        Retrieves a player achievement data from the database by player ID and achievement ID.
        
        Parameters
        ----------
        playerId : str | ObjectId
            The unique identifier of the player whose achievement data is to be retrieved.
        achievementId : str | ObjectId
            The unique identifier of the achievement whose data is to be retrieved.
            
        Returns
        -------
        dict
            The player achievement data corresponding to the given player ID and achievement ID, or None if not found.
        """
        if isinstance(playerId, str):
            playerId = ObjectId(playerId)
        if isinstance(achievementId, str):
            achievementId = ObjectId(achievementId)

        return self.player_achievements.find_one({'player_id': playerId, 'achievement_id': achievementId})

    
    # Creates a backup of the database in a json file
    def createBackup(self, file_path: str):
        """
        Creates a backup of the database in a JSON file.
        ...
        backup_data = {
            ...
            'Player_Achievements': list(self.player_achievements.find({}))
        }
        ...
        with open(file_path, 'w') as backup_file:
            json.dump(backup_data, backup_file, default=convert_objectid, indent=4)
